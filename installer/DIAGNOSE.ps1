...
.DESCRIPTION
    Answers the following questions from real build output (no guessing):
      ...
      Q5. Are there any external: / dynamic require() calls in the bundle?
      Q6. What is the package.json inside the asar?
      Q7. What called require('obuf')?
...
# ─── Colour helpers ────────────────────────────────────────────────────────────
function W-Title($t) { Write-Host "`n━━━ $t ━━━" -ForegroundColor Cyan }
...
$asarLines = $asarList | Where-Object { $_ -and $_.ToString().Trim() -ne '' } | ForEach-Object { $_.ToString() }
...
if ($nmEntries.Count -eq 0) {
    ...
    # Check for specific problem packages
...
if (Test-Path $ExtractDir) {
    ...
    if (Test-Path $pkgJsonPath) {
        ...
        W-Info "  dependencies (count): $(if($pkgJson.dependencies){($pkgJson.dependencies | Get-Member -MemberType NoteProperty).Count}else{0})"
    ...
    # node_modules in extracted asar
    $nmDir = "$ExtractDir\node_modules"
    if (Test-Path $nmDir) {
    ...
    # Check the actual main.js from inside the asar
    ...
    if (Test-Path $mainInAsar) {
        ...
        $checks = @{
            ...
            'require("obuf")'    = 'dynamic require("obuf")';
            "require('obuf')"    = "dynamic require('obuf')";
            'require("pg")'      = 'dynamic require("pg")';
            "require('pg')"      = "dynamic require('pg')";
            'Cannot find module' = 'error message (built-in to bundle?)';
        }
        W-Info "  Searching bundle for key strings:"
        foreach ($kv in $checks.GetEnumerator()) {
            if ($content -cmatch [regex]::Escape($kv.Key)) {
                W-Info "  [FOUND] '$($kv.Key)'  ← $($kv.Value)"
            } else {
                W-Info "  [MISS ] '$($kv.Key)'"
            }
        }

        # Count obuf occurrences
        $obufMatches = ([regex]::Matches($content, 'obuf')).Count
        W-Info "  Total 'obuf' occurrences in bundle: $obufMatches"

        # First 5 lines of bundle (to verify it's esbuild output)
        W-Info "  First 3 lines of main.js:"
...
if (Test-Path $BundlePath) {
    ...
    # Check for external require calls that would fail at runtime
    ...
    if ($runtimeRequires.Count -gt 0) {
        W-Warn "  External require() calls found in bundle (packages that are NOT built-in):"
    ...
    if ($obufCountDist -eq 0) {
        ...
        W-Fail "  This means pg loads obuf from node_modules at runtime — which fails"
    } else {
...
# ─── SECTION 5: esbuild command verification ──────────────────────────────────
...
# ─── SECTION 6: pnpm ls --prod (what pnpm considers production) ───────────────
...
Write-Host "  obuf in dist bundle     : $(if((Test-Path $BundlePath) -and (Get-Content $BundlePath -Raw) -match 'obuf'){'YES (correct)'}else{'NO (PROBLEM)'})"
Write-Host ""
