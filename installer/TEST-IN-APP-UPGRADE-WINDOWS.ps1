...
.USAGE
    PowerShell (Admin):
      .\TEST-IN-APP-UPGRADE-WINDOWS.ps1 `
        -InstallerExe "C:\Build\OneSoftSetup-1.0.41-x64.exe"

    InstallerExe is optional. When omitted, the script automatically locates
        OneSoftSetup-1.0.41-x64.exe next to this script, in Downloads, or on the
    Desktop.
...
param(
    ...
    [string]$ExpectedNewVersion = "1.0.41",
...
function Resolve-Installer {
    ...
    foreach ($directory in $searchDirectories) {
        $candidate = Join-Path $directory "OneSoftSetup-1.0.41-x64.exe"
...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($InstallerExe) -and (Test-Path $InstallerExe -PathType Leaf)) {
        ...
        Fail "OneSoftSetup-1.0.41-x64.exe was not found next to the script, in Downloads, or on the Desktop"
    }
