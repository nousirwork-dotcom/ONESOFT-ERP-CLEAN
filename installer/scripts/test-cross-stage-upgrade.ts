...
const serverSource = path.join(serverAppPath, 'src', 'index.ts');
const serverTsx = path.join(serverAppPath, 'node_modules', 'tsx', 'dist', 'cli.mjs');
...
function startBackend(portToCheck: number): ChildProcess {
  const entrypoint = fs.existsSync(serverDist)
    ? [serverDist]
    : [serverTsx, serverSource];
