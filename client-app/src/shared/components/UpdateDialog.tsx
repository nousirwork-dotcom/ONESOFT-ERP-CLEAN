/**
 * UpdateDialog — نافذة التحديث التلقائي لـ OneSoft ERP
 *
 * تدعم نوعين:
 *   optional  → زر "لاحقاً" + "تحديث الآن"
 *   mandatory → زر "تحديث الآن" فقط — لا يمكن إغلاق النافذة
 *
 * تصميم OneSoft:
 *   خلفية: #FAF7F0 (كريم)  |  كارت: أبيض  |  عنوان: #1B2B5C (navy)
 *   زر رئيسي: #1B2B5C  |  زر ثانوي: رمادي  |  تحذير إجباري: ذهبي/أحمر
 */
...
// ─── Electron bridge (فقط داخل Electron) ─────────────────────────────────────
...
export default function UpdateDialog() {
  ...
  useEffect(() => {
    ...
    const offDownloaded = updater.onUpdateDownloaded((_, data) => {
  ...
  // لا شيء يُعرض إذا لم يكن هناك تحديث
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "rgba(27,43,92,0.45)", backdropFilter: "blur(4px)" }}
      dir="rtl"
    >
      {/* Card */}
      <div
      ...
      >
        ...
        {isMandatory && (
          <div className="flex items-center gap-2 px-5 py-2.5"
            ...
            <span className="text-sm font-bold">تحديث إجباري مطلوب — لا يمكن متابعة العمل قبل التحديث</span>
          </div>
        )}
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {state === "error" && (
            <div className="p-3 rounded-xl text-sm"
              style={{ backgroundColor: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.2)", color: "#b91c1c" }}>
        ...
        <div className="flex items-center gap-3 px-6 pb-5 pt-2"
          ...
          {state === "downloaded" ? (
            ...
            <div className="flex-1 py-2.5 rounded-xl text-center text-sm font-semibold"
...
// ─── VersionBadge ─────────────────────────────────────────────────────────────
function VersionBadge({ label, version, dim, highlight }: {
    ...
    <div className="space-y-2">
      ...
      {progress && progress.bytesPerSecond > 0 && (
        <p className="text-xs text-center" style={{ color: "#6b7280" }}>
          سرعة التحميل: <strong>{fmtBytes(progress.bytesPerSecond)}/ث</strong>
        </p>
      )}
    </div>
  );
