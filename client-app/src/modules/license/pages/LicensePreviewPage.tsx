    ...
    <div className="bg-[#FAF7F0] rounded-xl border border-[#C9A84C]/20 px-4 py-3.5">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          <span className="text-[#1B2B5C]/70 w-5 h-5">{icon}</span>
          <span className="text-[14px] font-bold text-[#374151]">{label}</span>
        </div>
        <div className="flex items-baseline gap-1">
          <span className={`text-[26px] font-black leading-none ${tc}`}>{current}</span>
          <span className="text-[14px] text-[#9CA3AF] font-semibold"> / {max}</span>
        </div>
      </div>
...
function AccessBadge({ label, value, icon }: { label: string; value?: boolean | null; icon: React.ReactNode }) {
  const on = value === true;
  return (
    <div className={`flex items-center justify-between px-4 py-3 rounded-xl border text-[14px] font-semibold ${on ? "bg-green-50 border-green-200 text-green-800" : "bg-[#F4F0E8] border-[#E0D8CC] text-[#9CA3AF]"}`}>
      <div className="flex items-center gap-2.5">{icon}<span>{label}</span></div>
      <span className={`text-lg font-bold ${on ? "text-green-500" : "text-[#CBD5E1]"}`}>{on ? "✓" : "✕"}</span>
    </div>
  );
}
...
// ─── Activation Panel (shared across layouts) ────────────────────────────
function ActivationPanel({ isValid, ck, copy }: { isValid: boolean; ck: string | null; copy: (v: string, k: string) => void }) {
  const [tab, setTab] = useState<"code" | "file" | "trial">("code");
  const [activCode, setActivCode] = useState("");
  const TABS = [
    { key: "code"  as const, label: "كود التفعيل" },
    { key: "file"  as const, label: "استيراد ملف" },
    { key: "trial" as const, label: "فترة تجريبية" },
  ];
  return (
    <div>
      <SectionTitle icon={<KeyRound className="w-5 h-5" />} title="تفعيل الترخيص" />
      {/* Tabs */}
      ...
      {tab === "code" && (
        <div className="space-y-4">
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2.5 px-5 py-4 rounded-xl text-[16px] font-bold transition-colors shadow-md ${
      ...
      {tab === "file" && (
        <div className="space-y-4">
          <p className="text-[14px] text-[#6B7280] leading-relaxed">استورد ملف الترخيص (.ons) الصادر من مزود النظام.</p>
          <label className="block border-2 border-dashed border-[#C9A84C]/40 rounded-xl p-8 text-center cursor-pointer hover:bg-[#FAF7F0] transition-colors">
            <FileUp className="w-10 h-10 mx-auto text-[#C9A84C] mb-3" />
            <p className="text-[15px] font-bold text-[#4A5568]">اضغط لاختيار ملف الترخيص</p>
            <p className="text-[13px] text-[#9CA3AF] mt-1">.ons أو .json</p>
          </label>
          <button className="w-full flex items-center justify-center gap-2.5 px-5 py-4 rounded-xl bg-gray-200 text-gray-400 text-[16px] font-bold cursor-not-allowed">
        ...
        <div className="space-y-4">
          <div className="bg-[#FFF8E0] border border-[#C9A84C]/30 rounded-xl p-4">
            ...
            <p className="text-[13px] text-[#6B7280] leading-relaxed">
              للحصول على فترة تجريبية، تواصل مع مزود النظام وأرسل Device ID الخاص بجهازك، وسيُرسل لك ملف ترخيص تجريبي موقّع.
            </p>
          </div>
          <div className="space-y-3">
            {["انسخ Device ID أدناه", "أرسله لمزود النظام", "استلم ملف الترخيص التجريبي", "استورده من تبويب «استيراد»"].map((s, i) => (
              <div key={i} className="flex items-center gap-3 text-[14px] text-[#4A5568]">
                <span className="w-7 h-7 rounded-full bg-[#1B2B5C] text-white text-[11px] font-black flex items-center justify-center shrink-0">{i + 1}</span>
                {s}
              </div>
            ))}
          </div>
          <div className="flex gap-3 pt-1">
            <button onClick={() => copy(MOCK_DID, "did-t")}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-[#1B2B5C]/25 text-[#1B2B5C] text-[13px] font-bold hover:bg-[#1B2B5C]/5 transition-colors">
              {ck === "did-t" ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />} نسخ Device ID
            </button>
            <button className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-[#1B2B5C]/25 text-[#1B2B5C] text-[13px] font-bold hover:bg-[#1B2B5C]/5 transition-colors">
      ...
      {isValid && (
        <div className="mt-4 flex items-center gap-2.5 px-4 py-3 bg-green-50 border border-green-200 rounded-xl text-[14px] text-green-800 font-semibold">
...
// ─── Device Info Panel ────────────────────────────────────────────────────
function DevicePanel({ isValid, ck, copy }: { isValid: boolean; ck: string | null; copy: (v: string, k: string) => void }) {
  return (
    <div>
      <SectionTitle icon={<Fingerprint className="w-5 h-5" />} title="معلومات الجهاز" />
      <div className="space-y-3">
        <div className="bg-[#FAF7F0] rounded-xl border border-[#C9A84C]/20 px-4 py-3.5">
          <p className="text-[11px] font-bold text-[#C9A84C] uppercase tracking-wider mb-1.5">Device ID</p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-[13px] font-mono text-[#1B2B5C] break-all leading-snug">{MOCK_DID}</code>
            <button onClick={() => copy(MOCK_DID, "dv2")}
              className="p-2 rounded-lg border border-[#C9A84C]/30 text-[#C9A84C] hover:bg-white transition-colors shrink-0">
              {ck === "dv2" ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
            </button>
          </div>
        </div>
        <div className="bg-[#FAF7F0] rounded-xl border border-[#C9A84C]/20 px-4 py-3.5">
          <p className="text-[11px] font-bold text-[#C9A84C] uppercase tracking-wider mb-1.5">Request Code</p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-[13px] font-mono text-[#9CA3AF]">LIQ-XXXX-XXXX-XXXX</code>
            <button className="p-2 rounded-lg border border-[#C9A84C]/30 text-[#C9A84C] hover:bg-white transition-colors shrink-0">
              <Copy className="w-4 h-4" />
            </button>
          </div>
        </div>
        <div className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border text-[14px] font-semibold ${isValid ? "bg-green-50 border-green-200 text-green-800" : "bg-[#FAF7F0] border-[#E0D8CC] text-[#9CA3AF]"}`}>
          {isValid
            ? <><CheckCircle2 className="w-5 h-5 text-green-500 shrink-0" /> الجهاز مرتبط بترخيص صالح</>
            : <><XCircle className="w-5 h-5 shrink-0" /> الجهاز غير مرتبط بأي ترخيص</>
          }
        </div>
        <div className="flex items-start gap-2.5 px-4 py-3 bg-[#FAF7F0] rounded-xl border border-[#C9A84C]/20 text-[13px] text-[#6B7280]">
          <Phone className="w-4 h-4 text-[#C9A84C] shrink-0 mt-0.5" />
          استخدم كود الطلب عند التواصل مع مزود النظام للحصول على ملف الترخيص.
        </div>
      </div>
    </div>
  );
...
function LicenseScreen({ status, ck, copy }: {
  ...
  const noLicense  = isSlate || isInvalid;   // ← no payload, focus on activation
  ...
  const curPos      = isValid ? 2 : 0;
  const curDevices  = isValid ? 1 : 0;
  ...
  const badge = isValid && !isTrial
    ...
    : { label: "غير مفعّل",       bg: "bg-gray-100",   text: "text-gray-700",   bdr: "border-gray-300" };

  const ShIcon  = isValid && !isTrial ? ShieldCheck : isTrial ? ShieldCheck : isSlate ? ShieldQuestion : isExpired ? ShieldAlert : ShieldOff;
  const shCls   = isValid && !isTrial ? "text-green-500" : isTrial ? "text-[#C9A84C]" : "text-red-500";
  ...
  const daysClr = !days ? "" : days <= 7 ? "text-red-600" : days <= 30 ? "text-amber-600" : "text-[#1B2B5C]";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[24px] font-black text-[#1B2B5C] leading-none">الترخيص والتفعيل</h1>
          <p className="text-[14px] text-[#6B7280] mt-1.5">إدارة حالة ترخيص النظام وتفعيل النسخة</p>
        </div>
        <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-[#C9A84C]/40 bg-white text-[#1B2B5C] text-[14px] font-semibold shadow-sm hover:bg-[#FAF7F0] transition-colors">
      ...
      <Card className="p-6">
        <div className="flex items-start gap-6">
          ...
          <div className={`shrink-0 w-20 h-20 rounded-2xl flex items-center justify-center border-2 shadow-inner ${shBgCls}`}>
            <ShIcon className={`w-10 h-10 ${shCls}`} />
          </div>

          {/* Main info */}
          <div className="flex-1 min-w-0 space-y-2.5">
            <div className="flex items-center gap-3 flex-wrap">
              <span className={`inline-flex items-center px-4 py-1.5 rounded-full border text-[14px] font-bold ${badge.bg} ${badge.text} ${badge.bdr}`}>
                {badge.label}
              </span>
              {isValid && <span className="text-[14px] text-[#6B7280]">باقٍ من انتهاء الصلاحية</span>}
            </div>
            <div className="flex items-baseline gap-3">
              <h2 className="text-[30px] font-black text-[#1B2B5C] leading-none">{p?.package_name ?? typeLabel}</h2>
              {p?.package_name && <span className="text-[15px] text-[#6B7280] font-medium">{typeLabel}</span>}
            </div>
            {p?.customer_name && <p className="text-[15px] text-[#6B7280] font-medium">{p.customer_name}</p>}
            {statusMsg && (
              <div className={`flex items-start gap-2.5 rounded-xl px-4 py-3 border text-[14px] mt-2 ${
                isInvalid || isExpired ? "bg-red-50 border-red-200 text-red-800" : isSlate ? "bg-[#FFF8E0] border-[#C9A84C]/30 text-[#6B5100]" : "bg-[#FFF8E0] border-[#C9A84C]/30 text-[#6B5100]"
              }`}>
            ...
            {isInvalid && (
              <div className="flex items-start gap-2.5 rounded-xl px-4 py-3 border text-[13px] bg-amber-50 border-amber-200 text-amber-800">
          ...
          {/* Days counter — only if payload exists */}
          {p && (
            <div className={`shrink-0 flex flex-col items-center justify-center gap-1 rounded-2xl border-2 px-6 py-4 text-center min-w-[110px] ${
              ...
              {isLife ? (
                <><InfinityIcon className="w-10 h-10 text-[#1B2B5C]" /><p className="text-[12px] text-[#1B2B5C]/60 font-bold mt-1">دائم</p></>
              ) : (
                <>
                  <span className={`text-[44px] font-black leading-none ${daysClr || "text-[#1B2B5C]"}`}>{days}</span>
                  <span className="text-[13px] text-[#6B7280] font-bold">يوم</span>
                  {(days ?? 0) <= 30 && <span className={`text-[12px] font-black mt-0.5 ${(days ?? 0) <= 7 ? "text-red-600" : "text-amber-600"}`}>{(days ?? 0) <= 7 ? "⚠ عاجل" : "قريبًا"}</span>}
                </>
              )}
            </div>
          )}
        ...
        {/* IDs grid — only if payload */}
        {p && (
          <div className="mt-5 pt-4 border-t border-[#C9A84C]/20 grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-4">
            {[
              ...
              <div key={i} className="min-w-0">
                <p className="text-[11px] font-bold text-[#C9A84C] uppercase tracking-wider mb-1">{item.label}</p>
                <div className="flex items-center gap-1.5">
                  <span className="text-[13px] font-mono text-[#1B2B5C]/80 truncate">{item.value ?? "—"}</span>
                  {(item as any).id && item.value && (
                    <button onClick={() => copy(item.value!, (item as any).id)}
                      ...
                      {ck === (item as any).id ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                  )}
                </div>
              </div>
        ...
        {isTrial && p && (
          <div className="mt-4 p-4 bg-[#FFF8E0] rounded-xl border border-[#C9A84C]/30">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <span className="text-[13px] text-[#C9A84C]/70 mr-auto">قابلة للترقية إلى ترخيص كامل</span>
            </div>
            <div className="grid grid-cols-4 gap-4">
              <div><p className="text-[11px] text-[#C9A84C]/70 font-bold uppercase mb-1">المدة</p><p className="text-[22px] font-black text-[#1B2B5C]">{trialDuration} يوم</p></div>
              <div><p className="text-[11px] text-[#C9A84C]/70 font-bold uppercase mb-1">تاريخ البداية</p><p className="text-[14px] font-semibold text-[#1B2B5C]">{fmtDate(p.start_date)}</p></div>
              <div><p className="text-[11px] text-[#C9A84C]/70 font-bold uppercase mb-1">تاريخ النهاية</p><p className="text-[14px] font-semibold text-[#1B2B5C]">{fmtDate(p.expiry_date)}</p></div>
              <div><p className="text-[11px] text-[#C9A84C]/70 font-bold uppercase mb-1">الأيام المتبقية</p>
                <p className={`text-[32px] font-black leading-none ${(days ?? 0) <= 7 ? "text-red-600" : (days ?? 0) <= 14 ? "text-amber-600" : "text-[#1B2B5C]"}`}>{days}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {isExpired && p && (
          <div className="mt-4 p-4 bg-red-50 rounded-xl border border-red-200">
            ...
            <div className="grid grid-cols-3 gap-4 text-[14px]">
              <div><p className="text-[11px] text-red-400/70 font-bold uppercase mb-1">تاريخ البداية</p><p className="font-semibold text-red-800">{fmtDate(p.start_date)}</p></div>
              <div><p className="text-[11px] text-red-400/70 font-bold uppercase mb-1">تاريخ الانتهاء</p><p className="font-semibold text-red-800">{fmtDate(p.expiry_date)}</p></div>
              <div><p className="text-[11px] text-red-400/70 font-bold uppercase mb-1">الإجراء المطلوب</p><p className="font-semibold text-red-800">التواصل مع مزود النظام</p></div>
            </div>
          </div>
        )}
        ...
        {/* Active bar */}
        {isValid && !isTrial && (
          <div className="mt-4 flex items-center gap-2.5 px-5 py-3 bg-green-50 rounded-xl border border-green-200">
            <CheckCircle2 className="w-5 h-5 text-green-500 shrink-0" />
            <span className="text-[14px] font-semibold text-green-800">الترخيص مفعّل وجميع خدمات النظام تعمل بشكل طبيعي.</span>
          </div>
        )}
      </Card>

      {/* ════════════════════════════════════════════════════════════════
          INACTIVE / INVALID  →  Activation-focused layout
          ══════════════════════════════════════════════════════════════ */}
      {noLicense && (
        <div className="grid grid-cols-5 gap-5">

          {/* Activation card — takes 3/5 width */}
          <Card className="col-span-3 p-6">
            <ActivationPanel isValid={false} ck={ck} copy={copy} />
          </Card>

          {/* Right column: Device info + small modules note */}
      ...
      {/* ════════════════════════════════════════════════════════════════
          ACTIVE / TRIAL / EXPIRED  →  Full 3-column layout
          ══════════════════════════════════════════════════════════════ */}
      {!noLicense && (
        <div className="grid grid-cols-3 gap-5">
          ...
          <Card className="p-6">
            <SectionTitle icon={<span>🏅</span>} title="حدود الترخيص" />
            {p ? (
              <div className="space-y-3">
                <LimitCard label="المستخدمون"  current={curUsers}    max={p.max_users}    icon={<Users className="w-5 h-5" />} />
                <LimitCard label="الفروع"       current={curBranches} max={p.max_branches} icon={<GitBranch className="w-5 h-5" />} />
                <LimitCard label="نقاط البيع"  current={curPos}      max={p.max_pos}      icon={<MonitorSmartphone className="w-5 h-5" />} />
                <LimitCard label="الأجهزة"      current={curDevices}  max={p.max_devices}  icon={<Fingerprint className="w-5 h-5" />} />
                <div className="pt-1 space-y-2.5">
                  <AccessBadge label="الويب"         value={p.web_allowed}             icon={<Globe className="w-5 h-5" />} />
                  <AccessBadge label="سطح المكتب"    value={p.desktop_allowed ?? true}  icon={<Monitor className="w-5 h-5" />} />
                  <AccessBadge label="وضع الأوفلاين" value={p.offline_allowed}          icon={<WifiOff className="w-5 h-5" />} />
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center py-10 gap-3">
                <Lock className="w-12 h-12 text-[#C9A84C]/25" />
                <p className="text-[14px] text-[#9CA3AF] text-center">سيتم عرض الحدود بعد التفعيل</p>
              </div>
            )}
          </Card>
          ...
          <div className="space-y-5">
            ...
            <Card className="p-6">
              <DevicePanel isValid={isValid} ck={ck} copy={copy} />
            </Card>
          </div>

          {/* COL 3 (leftmost in RTL): تفعيل الترخيص */}
          <Card className="p-6">
            <ActivationPanel isValid={isValid} ck={ck} copy={copy} />
          </Card>
        </div>
      )}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#F2EDE4]" dir="rtl">
      ...
      <aside className="w-[220px] bg-[#1B2B5C] flex flex-col shrink-0 h-full shadow-2xl">
        ...
        <div className="px-4 py-3 border-t border-white/10">
          <div className="flex items-center gap-2 text-white/35 hover:text-white/60 cursor-pointer transition-colors">
            ...
            <span className="text-[13px] font-medium">مساعدة</span>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-t border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#C9A84C] flex items-center justify-center text-[#1B2B5C] font-black text-[11px] shrink-0">
              م ن
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[12px] font-bold text-white truncate">مدير النظام</p>
              <p className="text-[10px] text-white/40 truncate">{MOCKS[active].status.payload?.customer_name ?? "—"}</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <div className="bg-white border-t border-[#C9A84C]/20 px-5 py-2.5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-5 text-[12px] text-[#6B7280]">
            ...
            <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-green-400 inline-block"></span> متصل</span>
          </div>
          <div className="flex items-center gap-5 text-[12px] text-[#6B7280]">
