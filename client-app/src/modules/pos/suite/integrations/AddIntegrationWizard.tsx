...
const CUSTOM_META: IntegrationProviderMeta = {
  ...
  credentialFields: [
    ...
    { key: 'notes', label: 'ملاحظات للمطور', type: 'text', required: false, placeholder: 'راجع docs/my-adapter.md' },
  ],
};

function makeRow(overrides: Partial<ProductMapping> = {}): ProductMapping {
  return {
...
/** مربع حوار تأكيد التجاوز عند وجود أصناف غير مربوطة */
interface UnmappedGuardProps {
  count: number;
  onConfirm: () => void;
  onBack: () => void;
}
function UnmappedGuardDialog({ count, onConfirm, onBack }: UnmappedGuardProps) {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 440, width: '90%', direction: 'rtl', boxShadow: '0 8px 32px rgba(0,0,0,.18)' }}>
        <h3 style={{ margin: '0 0 10px', color: 'var(--pos-text)', fontSize: 17 }}>⚠ أصناف غير مربوطة</h3>
        <p style={{ margin: '0 0 18px', color: 'var(--pos-muted)', fontSize: 13, lineHeight: 1.7 }}>
          يوجد <strong>{count} صنف</strong> لم يُربط بعد بكتالوج OneSoft.
          طلبات المنصة التي تحتوي هذه الأصناف ستُصنَّف تلقائياً <strong>"يحتاج مراجعة"</strong>.
        </p>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button type="button" className="pos-button pos-button--secondary" onClick={onBack}>← العودة للربط</button>
          <button type="button" className="pos-button pos-button--danger" onClick={onConfirm}>تجاوز والحفظ</button>
        </div>
      </div>
    </div>
  );
...
export function AddIntegrationWizard({ open, onClose, onConnected }: Props) {
  ...
  const [settings, setSettings] = useState<IntegrationConnectionSettings>({
    branchName: '', posName: '', warehouseId: null, journalId: null, defaultCustomerId: null,
    defaultPaymentMethod: 'حسب طلب المنصة', autoAccept: false, autoSendToKitchen: true,
    cancelHandling: 'notify_only', soundAlert: true, arrivalNotification: true,
  });
  ...
  const filteredRows = rows.filter((r) => {
    ...
    const matchSearch = !q || r.externalProductName.toLowerCase().includes(q) || r.externalProductCode.toLowerCase().includes(q) || (r.onesoftProductName ?? '').toLowerCase().includes(q);
    const matchFilter = mappingFilter === 'all' || (mappingFilter === 'mapped' && r.onesoftProductId !== null) || (mappingFilter === 'unmapped' && r.onesoftProductId === null);
  ...
  const handleAddRow = () => {
    ...
    setRows((prev) => [...prev, makeRow({ externalProductId: code || `ext-${Date.now()}`, externalProductCode: code, externalProductName: newName.trim(), externalPrice: parseFloat(newPrice) || 0 })]);
  ...
  const handleAutoMatch = () => {
    setRows((prev) => prev.map((r) => {
      ...
      const match = products.find((p) => p.name.toLowerCase() === r.externalProductName.toLowerCase() || p.code.toLowerCase() === r.externalProductCode.toLowerCase());
      return match ? { ...r, onesoftProductId: match.id, onesoftProductCode: match.code, onesoftProductName: match.name } : r;
    }));
  };
  ...
  const handleLinkProduct = (rowId: string | undefined, productId: number | null) => {
    ...
    setRows((prev) => prev.map((r) => r.rowId === rowId ? { ...r, onesoftProductId: productId, onesoftProductCode: linked?.code, onesoftProductName: linked?.name } : r));
  };

  const handleUnlink = (rowId: string | undefined) => {
    if (!rowId) return;
    setRows((prev) => prev.map((r) => r.rowId === rowId ? { ...r, onesoftProductId: null, onesoftProductCode: undefined, onesoftProductName: undefined } : r));
  };
  ...
  const handleReset = () => {
    setStep(0); setSelectedMeta(null); setCustomName(''); setCustomShortName(''); setCustomInitial(''); setCustomDesc(''); setCustomActive(true);
    setSettings({ branchName: '', posName: '', warehouseId: null, journalId: null, defaultCustomerId: null, defaultPaymentMethod: 'حسب طلب المنصة', autoAccept: false, autoSendToKitchen: true, cancelHandling: 'notify_only', soundAlert: true, arrivalNotification: true });
    setCredentials({}); setAdvancedOpen(false); setTesting(false); setTestResult(null);
    setRows([]); setMappingSearch(''); setMappingFilter('all'); setNewCode(''); setNewName(''); setNewPrice(''); setShowGuard(false);
  };
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const result = adapter ? await adapter.testConnection(credentials) : await (async () => { await new Promise((r) => setTimeout(r, 600)); return { success: true, message: 'تم إضافة التكامل المخصص.' }; })();
  ...
  const handleConfirmClick = () => {
    if (unmappedCount > 0 && rows.length > 0) {
      setShowGuard(true);
    } else {
      doConfirm();
    }
  ...
  /** حقل بيانات اتصال — يدعم select حقيقي */
  function CredentialField({ field, adminSection }: { field: (typeof credentialFields)[0]; adminSection?: boolean }) {
    return (
      <label key={field.key}>
        <span>
          {field.label}
          {field.required && <strong style={{ color: 'var(--pos-red)', marginRight: 4 }}>*</strong>}
          {adminSection && <em style={{ color: 'var(--pos-muted)', fontSize: 10, marginRight: 6 }}>(مدير)</em>}
        </span>
        {field.type === 'select' ? (
          <select
  ...
  return (
    <>
      <Modal open={open} title="إضافة تكامل جديد" onClose={handleClose} width={860}>
            ...
            <div>
              <div className="pos-wizard-provider-info">
                ...
                <div className="pos-hub-card__status" style={{ marginRight: 'auto' }}>
                  ...
                  <span style={{ fontSize: 12 }}>{testResult ? (testResult.success ? 'متصل' : 'فشل') : 'لم يُختبر'}</span>
                </div>
              </div>

              {/* حقول عامة — تدعم select حقيقي */}
              {publicFields.length > 0 && (
                <div className="pos-form-grid" style={{ marginBottom: 14 }}>
                  {publicFields.map((field) => <CredentialField key={field.key} field={field} />)}
                </div>
              )}
                ...
                <div className="pos-collapsible">
                  ...
                  {advancedOpen && (
                    <div className="pos-collapsible__body">
                      ...
                      <div className="pos-form-grid">
                        {adminFields.map((field) => <CredentialField key={field.key} field={field} adminSection />)}
                      </div>
                    </div>
                  )}
                </div>
          ...
          {step === 2 && activeMeta && (
            <div>
              ...
              <div className="pos-toggle-list" style={{ marginTop: 14 }}>
                {([['autoAccept', 'قبول تلقائي للطلبات'], ['autoSendToKitchen', 'إرسال تلقائي للمطبخ بعد القبول'], ['soundAlert', 'تنبيه صوتي عند وصول طلب جديد'], ['arrivalNotification', 'إشعار عند وصول طلب جديد']] as const).map(([key, label]) => (
                  <label key={key} className="pos-toggle">
                    <input type="checkbox" checked={!!(settings[key as keyof IntegrationConnectionSettings])} onChange={(e) => setSettings((s) => ({ ...s, [key]: e.target.checked }))} />
                    <i /><span>{label}</span>
                  </label>
                ))}
              </div>
          ...
          {step === 3 && activeMeta && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr auto', gap: 8, marginBottom: 12, padding: 12, background: '#f8fafc', borderRadius: 12, border: '1px solid var(--pos-border)' }}>
                ...
                <input placeholder="اسم الصنف على المنصة *" value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleAddRow(); }} style={{ minHeight: 38, border: '1px solid var(--pos-border)', borderRadius: 8, padding: '0 10px', font: 'inherit', fontSize: 12 }} />
                      ...
                      {filteredRows.map((r) => (
                        <tr key={r.rowId} className={r.onesoftProductId === null ? 'is-unmapped' : ''}>
                          ...
                          <td style={{ display: 'flex', gap: 4 }}>
                            {r.onesoftProductId !== null && (
                              <button type="button" className="pos-button pos-button--secondary" style={{ minHeight: 28, padding: '0 7px', fontSize: 10 }} onClick={() => handleUnlink(r.rowId)}>إلغاء الربط</button>
                            )}
                            <button type="button" className="pos-button pos-button--danger" style={{ minHeight: 28, padding: '0 7px', fontSize: 10 }} onClick={() => handleDeleteRow(r.rowId)}>حذف</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="pos-wizard-footer">
                ...
                <button type="button" className="pos-button pos-button--primary" onClick={handleConfirmClick}>
                  {rows.length === 0 ? 'تخطي والحفظ' : unmappedCount === 0 ? 'تأكيد الربط ✓' : `حفظ (${unmappedCount} غير مربوط)`}
                </button>
              </div>
            </div>
          )}
      ...
      {/* حارس الأصناف غير المربوطة — dialog منفصل عن مودال الويزارد */}
      {showGuard && (
        <UnmappedGuardDialog
          count={unmappedCount}
          onConfirm={() => { setShowGuard(false); doConfirm(); }}
          onBack={() => setShowGuard(false)}
        />
      )}
    </>
  );
