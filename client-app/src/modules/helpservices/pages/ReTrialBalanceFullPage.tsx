 ...
 * 4 views: list | detail (balance sheet) | accountTree | taxReturn
 */
...
import {
  ...
  Printer, FileSpreadsheet, Download, Save, X, Ban, ChevronLeft,
...
export default function ReTrialBalanceFullPage() {
  ...
  // Views: 'list' | 'detail' | 'accountTree' | 'taxReturn' | 'auditLog' | 'review' | 'settlement'
  ...
  // Update draft entries when DB changes
  useEffect(() => {
    if (entriesQ.data && Object.keys(draftEntries).length === 0) {
      const m: Record<number, any> = {};
      for (const e of entriesQ.data) {
        m[e.accountId] = {
          openingDebit: toNum(e.openingDebit),
          openingCredit: toNum(e.openingCredit),
  ...
  // Update tax form when data arrives
  useEffect(() => {
        ...
        <div className="flex items-center justify-between p-3 border-b" style={{ borderColor: C.border }}>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder={ar ? "بحث..." : "Search..."}
                className="pl-7 pr-2 py-1 text-xs border rounded w-48" style={{ borderColor: C.border }} />
            </div>
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr style={{ background: C.header }}>
                ...
                <th className="px-2 py-1.5 font-semibold border text-center" style={{ borderColor: C.border }}>{ar ? "الحسابات" : "Accounts"}</th>
                <th className="px-2 py-1.5 font-semibold border text-center" style={{ borderColor: C.border }}>{ar ? "إدارة" : "Actions"}</th>
              </tr>
            </thead>
            <tbody>
              {listQ.isLoading && (
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">{ar ? "جاري التحميل..." : "Loading..."}</td></tr>
              )}
              {!listQ.isLoading && items.length === 0 && (
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">{ar ? "لا توجد بيانات" : "No records"}</td></tr>
              )}
              {items.map((item, idx) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>
                    {acctsQ.data && selTB === item.id ? acctsQ.data.length : "—"}
                  </td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>
                    <div className="flex items-center justify-center gap-1">
                      <Button size="sm" variant="ghost" onClick={() => { setSelTB(item.id); setView("detail"); setDraftEntries({}); setUnsaved(false); }}>
                        <Eye className="w-3.5 h-3.5" />
                      </Button>
                      {canEdit && <Button size="sm" variant="ghost" onClick={() => { setEditId(item.id); setForm({ name: item.name, periodLabel: item.periodLabel ?? "", fromDate: item.fromDate ? item.fromDate.slice(0, 10) : "", toDate: item.toDate ? item.toDate.slice(0, 10) : "", projectId: item.projectId, scope: item.scope, notes: item.notes ?? "" }); setShowForm(true); }}>
                        <Pencil className="w-3.5 h-3.5" />
                      </Button>}
                      {canDelete && <Button size="sm" variant="ghost" className="text-red-600 hover:text-red-700" onClick={() => setShowDel(item.id)}>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs font-medium text-gray-600">{ar ? "من" : "From"}</label>
                    <input type="date" value={form.fromDate} onChange={(e) => setForm({ ...form, fromDate: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600">{ar ? "إلى" : "To"}</label>
                    <input type="date" value={form.toDate} onChange={(e) => setForm({ ...form, toDate: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-4">
              <div className="flex items-center gap-2 mb-3 text-red-600">
                ...
                <span className="font-semibold text-sm">{ar ? "تأكيد الحذف" : "Confirm Delete"}</span>
              </div>
              <p className="text-xs text-gray-600 mb-3">{ar ? "هل أنت متأكد من حذف هذا الميزان ؟ لا يمكن التراجع." : "Are you sure you want to delete this trial balance? This cannot be undone."}</p>
              <div className="flex justify-end gap-2">
                <Button size="sm" variant="outline" onClick={() => setShowDel(null)}>{ar ? "إلغاء" : "Cancel"}</Button>
                <Button size="sm" variant="destructive" onClick={() => deleteTB.mutate(showDel)}>{ar ? "حذف" : "Delete"}</Button>
              </div>
            </div>
          </div>
        )}
  ...
  if (view === "detail") {
    ...
    const handleEntryChange = (accountId: number, field: string, val: string) => {
      ...
      setDraftEntries(prev => {
        ...
        const od = toNum(next[accountId].openingDebit);
        const oc = toNum(next[accountId].openingCredit);
    ...
    const handleSaveEntries = () => {
      ...
      const payload = Object.entries(draftEntries).map(([aid, vals]) => ({
        accountId: +aid,
        openingDebit: toNum(vals.openingDebit),
        openingCredit: toNum(vals.openingCredit),
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between p-2 border-b" style={{ borderColor: C.border }}>
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <Button size="sm" variant="outline" onClick={handleSaveEntries} disabled={!unsaved || saveEntries.isLoading}>
              <Save className="w-3.5 h-3.5 mr-1" />{ar ? "حفظ" : "Save"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setView("taxReturn")}>
              <Receipt className="w-3.5 h-3.5 mr-1" />{ar ? "الضريبة" : "Tax"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setView("review")}>
              <ClipboardCheck className="w-3.5 h-3.5 mr-1" />{ar ? "المراجعة" : "Review"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setView("accountTree")}>
              <FolderTree className="w-3.5 h-3.5 mr-1" />{ar ? "الدليل" : "Accounts"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setView("auditLog")}>
              <History className="w-3.5 h-3.5 mr-1" />{ar ? "السجل" : "Log"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => setView("settlement")}>
              <Settings2 className="w-3.5 h-3.5 mr-1" />{ar ? "التسوية" : "Settlement"}
            </Button>
            {canChart && (
              <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={() => { if (confirm(ar ? "إعادة الدليل الافتراضي ؟ سيمحى جميع البيانات." : "Reset to default accounts? All data will be cleared.")) resetDefaults.mutate(selTB!); }}>
                <RotateCcw className="w-3.5 h-3.5 mr-1" />{ar ? "إعادة" : "Reset"}
              </Button>
            )}
            {canExport && (
              <Button size="sm" variant="outline" onClick={() => window.open(`/api/trpc/reTrialBalance.exportTrialBalance?input=${encodeURIComponent(JSON.stringify({ trialBalanceId: selTB, format: "csv" }))}`, "_blank")}>
                <FileSpreadsheet className="w-3.5 h-3.5 mr-1" />{ar ? "CSV" : "CSV"}
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 border-b bg-gray-50" style={{ borderColor: C.border }}>
          ...
          {Math.abs(diff) < 0.01 ? (
            <span className="text-xs px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 font-medium">{ar ? "متزان ✓" : "Balanced ✓"}</span>
          ) : (
            <span className="text-xs px-2 py-0.5 rounded bg-red-100 text-red-700 font-medium">{ar ? `غير متزان → فارق ${fmt2(Math.abs(diff))}` : `Unbalanced → Diff ${fmt2(Math.abs(diff))}`}</span>
          )}
          <span className="text-xs text-gray-400">{ar ? `الإقفال دائن: ${fmt2(total.endingDebit)}   ج. ${fmt2(total.endingCredit)}` : `End Debit: ${fmt2(total.endingDebit)}   Credit: ${fmt2(total.endingCredit)}`}</span>
        </div>
          ...
          {bsQ.isLoading ? (
            ...
            <table className="w-full text-xs border-collapse">
              <thead className="sticky top-0 z-10">
                <tr style={{ background: C.header }}>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border, width: 40 }}>#</th>
                  <th className="px-2 py-1.5 border font-semibold text-left" style={{ borderColor: C.border }}>{ar ? "الحساب" : "Account"}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "رأسي د." : "Op. Dr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "رأسي ج." : "Op. Cr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "حركة د." : "Mov. Dr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "حركة ج." : "Mov. Cr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "إقفال د." : "End Dr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border }}>{ar ? "إقفال ج." : "End Cr."}</th>
                  <th className="px-2 py-1.5 border font-semibold text-center" style={{ borderColor: C.border, width: 50 }}>{ar ? "المراجعة" : "Rev."}</th>
                </tr>
              </thead>
              <tbody>
                {bs?.rows?.map((r, idx) => {
                  ...
                  const od = draft ? toNum(draft.openingDebit) : toNum(e?.openingDebit);
                  const oc = draft ? toNum(draft.openingCredit) : toNum(e?.openingCredit);
                  ...
                  return (
                    <tr key={a.id} className={`${a.isSystem ? "bg-gray-50/50" : "bg-white"} hover:bg-blue-50/50`}>
                      ...
                      <td className="px-2 py-1 border" style={{ borderColor: C.border }}>
                        <div className="font-medium text-gray-800">{a.code} · {a.name}</div>
                        <div className="text-[10px] text-gray-400">{CAT_LABELS[a.category]?.[lang] ?? a.category} · {a.nature === "debit" ? (ar ? "دائن" : "Dr") : (ar ? "دائن" : "Cr")}</div>
                      </td>
                      <td className="px-1 py-1 border text-center" style={{ borderColor: C.border }}>
                        <input type="number" min={0} step="0.01"
                          value={draft ? draft.openingDebit : (toNum(e?.openingDebit) || "")}
                          onChange={(ev) => handleEntryChange(a.id, "openingDebit", ev.target.value)}
                          className="w-20 text-xs text-center border rounded py-0.5 px-1" style={{ borderColor: C.border }} />
                      </td>
                      <td className="px-1 py-1 border text-center" style={{ borderColor: C.border }}>
                        <input type="number" min={0} step="0.01"
                          value={draft ? draft.openingCredit : (toNum(e?.openingCredit) || "")}
                          onChange={(ev) => handleEntryChange(a.id, "openingCredit", ev.target.value)}
                          className="w-20 text-xs text-center border rounded py-0.5 px-1" style={{ borderColor: C.border }} />
                      </td>
                      <td className="px-1 py-1 border text-center" style={{ borderColor: C.border }}>
                        <input type="number" min={0} step="0.01"
                          value={draft ? draft.movementDebit : (toNum(e?.movementDebit) || "")}
                          onChange={(ev) => handleEntryChange(a.id, "movementDebit", ev.target.value)}
                          className="w-20 text-xs text-center border rounded py-0.5 px-1" style={{ borderColor: C.border }} />
                      </td>
                      <td className="px-1 py-1 border text-center" style={{ borderColor: C.border }}>
                        <input type="number" min={0} step="0.01"
                          value={draft ? draft.movementCredit : (toNum(e?.movementCredit) || "")}
                ...
                {/* Totals row */}
                <tr style={{ background: C.header }} className="font-semibold">
                  <td colSpan={2} className="px-2 py-1.5 border text-right" style={{ borderColor: C.border }}>{ar ? "المجموع" : "TOTAL"}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.openingDebit)}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.openingCredit)}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.movementDebit)}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.movementCredit)}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.endingDebit)}</td>
                  <td className="px-2 py-1.5 border text-center" style={{ borderColor: C.border }}>{fmt2(total.endingCredit)}</td>
                  <td className="px-2 py-1.5 border" style={{ borderColor: C.border }} />
                </tr>
                {/* Difference row */}
                <tr>
                  <td colSpan={2} className="px-2 py-1.5 border text-right font-semibold" style={{ borderColor: C.border }}>{ar ? "الفارق" : "DIFFERENCE"}</td>
                  <td colSpan={4} className="border" style={{ borderColor: C.border }} />
                  <td className={`px-2 py-1.5 border text-center font-bold ${diff >= 0 ? "" : "text-red-600"}`} style={{ borderColor: C.border }}>{diff >= 0 ? fmt2(diff) : ""}</td>
                  <td className={`px-2 py-1.5 border text-center font-bold ${diff < 0 ? "text-red-600" : ""}`} style={{ borderColor: C.border }}>{diff < 0 ? fmt2(-diff) : ""}</td>
                  <td className="border" style={{ borderColor: C.border }} />
                </tr>
              </tbody>
            </table>
          )}
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between p-2 border-b" style={{ borderColor: C.border }}>
          ...
          {canChart && <Button size="sm" onClick={() => { setAcctForm({ code: "", name: "", category: "assets", nature: "debit", sortOrder: (accounts.length + 1) * 10, parentId: null }); setAcctEditId(null); setShowAcctForm(true); }}>
            <Plus className="w-3.5 h-3.5 mr-1" />{ar ? "حساب" : "Account"}
          </Button>}
        </div>
        <div className="flex-1 overflow-auto p-2">
          <table className="w-full text-xs border-collapse">
              ...
              {accounts.map((a, idx) => (
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-2 py-1 border text-center" style={{ borderColor: C.border }}>
                    {canChart && (
                      <div className="flex items-center justify-center gap-1">
                        <Button size="sm" variant="ghost" onClick={() => { setAcctEditId(a.id); setAcctForm({ code: a.code, name: a.name, category: a.category, nature: a.nature as "debit" | "credit", sortOrder: a.sortOrder, parentId: a.parentId }); setShowAcctForm(true); }}>
                          <Pencil className="w-3.5 h-3.5" />
                        </Button>
                        {!a.isSystem && (
                          <Button size="sm" variant="ghost" className="text-red-600" onClick={() => { if (confirm(ar ? "تأكيد الحذف؟" : "Confirm delete?")) deleteAcct.mutate(a.id); }}>
                            <Trash2 className="w-3.5 h-3.5" />
                          </Button>
                        )}
                      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs font-medium text-gray-600">{ar ? "التصنيف" : "Category"}</label>
                    <select value={acctForm.category} onChange={(e) => setAcctForm({ ...acctForm, category: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }}>
                      {Object.entries(CAT_LABELS).map(([k, v]) => <option key={k} value={k}>{v[lang]}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-600">{ar ? "الطبيعة" : "Nature"}</label>
                    <select value={acctForm.nature} onChange={(e) => setAcctForm({ ...acctForm, nature: e.target.value as "debit" | "credit" })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }}>
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-auto p-4 max-w-2xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "الفترة" : "Period"}</label>
              <input value={tax.periodLabel ?? ""} onChange={(e) => setTaxForm({ ...tax, periodLabel: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "الحالة" : "Refund Status"}</label>
              <select value={tax.refundStatus ?? "not_submitted"} onChange={(e) => setTaxForm({ ...tax, refundStatus: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }}>
                <option value="not_submitted">{ar ? "لم يتم التقديم" : "Not submitted"}</option>
                <option value="under_review">{ar ? "قيد المراجعة" : "Under review"}</option>
                <option value="approved">{ar ? "معتمد" : "Approved"}</option>
                <option value="refunded">{ar ? "مسدد" : "Refunded"}</option>
                <option value="offset">{ar ? "متصل" : "Offset"}</option>
              </select>
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "قيمة المشتريات (بدون ضريبة)" : "Purchases Pre-Tax"}</label>
              <input type="number" step="0.01" value={tax.purchasesPreTax ?? 0} onChange={(e) => setTaxForm({ ...tax, purchasesPreTax: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "مردودات المشتريات" : "Purchase Returns"}</label>
              <input type="number" step="0.01" value={tax.purchaseReturns ?? 0} onChange={(e) => setTaxForm({ ...tax, purchaseReturns: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "المشتريات الصافية" : "Net Purchases"}</label>
              <input type="number" step="0.01" value={tax.netPurchases ?? 0} onChange={(e) => setTaxForm({ ...tax, netPurchases: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "الضريبة المستحقة" : "Deductible Tax"}</label>
              <input type="number" step="0.01" value={tax.deductibleTax ?? 0} onChange={(e) => setTaxForm({ ...tax, deductibleTax: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "رصيد ضريبة افتتاحي" : "Opening Tax Balance"}</label>
              <input type="number" step="0.01" value={tax.openingTaxBalance ?? 0} onChange={(e) => setTaxForm({ ...tax, openingTaxBalance: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "المسدد الفعلي" : "Actual Refund"}</label>
              <input type="number" step="0.01" value={tax.actualRefund ?? 0} onChange={(e) => setTaxForm({ ...tax, actualRefund: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">{ar ? "المتصل الفعلي" : "Actual Offset"}</label>
              <input type="number" step="0.01" value={tax.actualOffset ?? 0} onChange={(e) => setTaxForm({ ...tax, actualOffset: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
            </div>
          </div>
          <div className="mt-3">
            <label className="text-xs font-medium text-gray-600">{ar ? "ملاحظات" : "Notes"}</label>
            <textarea rows={3} value={tax.notes ?? ""} onChange={(e) => setTaxForm({ ...tax, notes: e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
          </div>
          {/* Summary */}
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between p-2 border-b" style={{ borderColor: C.border }}>
          ...
          <div className="flex items-center gap-2">
            ...
            {canReview && <Button size="sm" onClick={() => {
              const payload = items.map(i => ({ accountId: i.account.id, reviewStatus: i.reviewStatus }));
              saveReview.mutate({ trialBalanceId: selTB!, reviews: payload });
            }}>
              <Save className="w-3.5 h-3.5 mr-1" />{ar ? "حفظ الحالة" : "Save"}
            </Button>}
          </div>
        </div>
        <div className="flex-1 overflow-auto p-2">
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-2 py-1 border" style={{ borderColor: C.border }}>
                      ...
                      <div className="text-[10px] text-gray-400">{CAT_LABELS[a.category]?.[lang] ?? a.category}</div>
                    </td>
                    ...
                    <td className="px-2 py-1 border text-center" style={{ borderColor: C.border }}>
                      <span className={`inline-block px-2 py-0.5 rounded text-[10px] ${label.color}`}>{label[lang]}</span>
                    </td>
                    {canReview && (
                      <td className="px-2 py-1 border text-center" style={{ borderColor: C.border }}>
                        <select value={item.reviewStatus} onChange={(e) => {
                          const newStatus = e.target.value;
                          // Update local state by mutating reviewQ data (not ideal but works for now)
                          const updated = items.map(i => i.account.id === a.id ? { ...i, reviewStatus: newStatus } : i);
                          // We can't mutate query cache directly; we rely on saveReview to persist
                          // Instead, show toast and let user save
                          toast.info(ar ? `تم تعديل المراجعة لـ ${a.name}. اضغط حفظ.` : `Review changed for ${a.name}. Click Save.`);
                        }} className="text-xs border rounded py-0.5 px-1" style={{ borderColor: C.border }}>
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          <div className="p-3 rounded border mb-4" style={{ borderColor: C.border, background: Math.abs(diff) < 0.01 ? "#f0fdf4" : "#fef2f2" }}>
            ...
            {Math.abs(diff) < 0.01 ? (
              <div className="text-sm text-emerald-700 font-bold">{ar ? "الميزان متزان ✓" : "Balanced ✓"}</div>
            ) : (
              <div className="text-sm text-red-700 font-bold">{ar ? `فارق: ${fmt2(Math.abs(diff))} ${diff > 0 ? "دائن" : "دائن"}` : `Difference: ${fmt2(Math.abs(diff))} ${diff > 0 ? "Dr" : "Cr"}`}</div>
            )}
          </div>
          ...
          {Math.abs(diff) >= 0.01 && (
            <>
              {!allReviewed && (
                <div className="mb-3 p-2 rounded bg-amber-50 border border-amber-200 text-xs text-amber-700">
                  <AlertTriangle className="w-4 h-4 inline mr-1" />
                  {ar ? "يجب مراجعة جميع الحسابات قبل التسوية" : "All accounts must be reviewed before settlement"}
                </div>
              )}
              {canSettle && allReviewed && (
                <div className="mb-4 p-3 rounded border bg-gray-50" style={{ borderColor: C.border }}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div>
                      <label className="text-xs text-gray-500">{ar ? "الحساب" : "Account"}</label>
                      <select value={settleForm.accountId ?? ""} onChange={(e) => setSettleForm({ ...settleForm, accountId: e.target.value ? +e.target.value : null })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }}>
                        <option value="">{ar ? "اختر..." : "Select..."}</option>
                        {ownerAccounts.map((a: any) => <option key={a.id} value={a.id}>{a.code} · {a.name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="text-xs text-gray-500">{ar ? "الفارق" : "Difference"}</label>
                      <input type="number" step="0.01" value={settleForm.difference} onChange={(e) => setSettleForm({ ...settleForm, difference: +e.target.value })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }} />
                    </div>
                    <div>
                      <label className="text-xs text-gray-500">{ar ? "الاتجاه" : "Direction"}</label>
                      <select value={settleForm.direction} onChange={(e) => setSettleForm({ ...settleForm, direction: e.target.value as "debit" | "credit" })} className="w-full text-xs border rounded px-2 py-1" style={{ borderColor: C.border }}>
                        <option value="debit">{ar ? "دائن" : "Debit"}</option>
                        <option value="credit">{ar ? "دائن" : "Credit"}</option>
                      </select>
                    </div>
                  ...
                  <Button size="sm" onClick={() => {
                    if (!settleForm.accountId) { toast.error(ar ? "اختر حساب" : "Select account"); return; }
                    saveSettle.mutate({ trialBalanceId: selTB!, data: { ...settleForm, accountId: settleForm.accountId, prevBalanceDebit: 0, prevBalanceCredit: 0, newBalanceDebit: settleForm.direction === "debit" ? settleForm.difference : 0, newBalanceCredit: settleForm.direction === "credit" ? settleForm.difference : 0, userConfirmed: true } });
                  }}>{ar ? "تسوية" : "Settle"}</Button>
                </div>
              )}
            </>
          )}
            ...
            <tbody>
              {settlements.map((s, idx) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-2 py-1 border" style={{ borderColor: C.border }}>
                    {(() => {
                      const a = acctsQ.data?.find((x: any) => x.id === s.accountId);
                      return a ? `${a.code} · ${a.name}` : `#${s.accountId}`;
                    })()}
                  </td>
                ...
                <tr key={log.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-2 py-1 border" style={{ borderColor: C.border }}>
                    <span className={`inline-block px-2 py-0.5 rounded text-[10px] ${
                      ...
                      log.action === "settlement" ? "bg-amber-100 text-amber-700" :
