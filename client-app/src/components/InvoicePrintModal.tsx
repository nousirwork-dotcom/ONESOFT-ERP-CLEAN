...
/* ═══════════════════ Compute line values ═══════════════════ */
function computeLines(data: PrintInvoiceData) {
  return data.lines.map(ln => {
    ...
    return { preDisc, discAmt, taxable, taxAmt, totalWithTax };
  });
}

function getColVal(key: ColKey, ln: PrintInvoiceData["lines"][0], calc: ReturnType<typeof computeLines>[0], idx: number): string {
  switch (key) {
